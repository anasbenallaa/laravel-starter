  ...
  return (
    ...
    >
      {isMobile || state === "collapsed" ? (
        <Icon iconNode={SidebarLeftIcon} className="rtl:-scale-x-100" />
      ) : (
        <Icon iconNode={SidebarLeft01Icon} className="rtl:-scale-x-100" />
      )}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-s px-2.5 rtl:-translate-x-px py-0.5",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
